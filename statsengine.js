...
const HERO_NAMES = [
    'pharah', 'reaper', 'soldier', 'reinhardt', 'junkrat', 'mei', 'tracer', 'genji', 'mccree', 'doomfist', 'winston',
    ...
    'sojourn', 'junkerqueen', 'kiriko', 'ramattra', 'lifeweaver', 'illari', 'mauga', 'venture', 'juno', 'hazard',
    'anran', 'dmon', 'domina', 'emre', 'freja', 'jetpackcat', 'mizuki', 'shion', 'sierra', 'vendetta', 'wuyang'
];
const HERO_NAMES_CLEAN = HERO_NAMES;
const HERO_NAMES_FRIENDLY = [
    'Pharah', 'Reaper', 'Soldier', 'Reinhardt', 'Junkrat', 'Mei', 'Tracer', 'Genji', 'Doomfist', 'McCree', 'Winston',
    ...
    'Sojourn', 'Junker Queen', 'Kiriko', 'Ramattra', 'Lifeweaver', 'Illari', 'Mauga', 'Venture', 'Juno', 'Hazard',
    'Anran', 'D.Mon', 'Domina', 'Emre', 'Freja', 'Jetpack Cat', 'Mizuki', 'Shion', 'Sierra', 'Vendetta', 'Wuyang'
];
...
function getImportantFieldsFor(hero, playMode) {
    ...
    if (hero == 'pharah') {
        ...
        fields = [
            ...
            {name: 'virus_kills_avg_per_10_min', prettyName: 'Virus Kills Per 10 Min', weight: 1.2, required: false},
            {name: 'weapon_accuracy', prettyName: 'Weapon Accuracy', weight: 1.5, required: false},
            {name: 'low_health_teleports_avg_per_10_min', prettyName: 'Low Health Escapes Per 10 Min', weight: 1.0, required: false},
        ...
        fields = [
            ...
            {name: 'final_blows_avg_per_10_min', prettyName: 'Final Blows Per 10 Min', weight: 1.5, required: false},
            {name: 'hero_damage_done_avg_per_10_min', prettyName: 'Hero Damage Per 10 Min', weight: 1.5, required: false},
            {name: 'barrier_damage_done_avg_per_10_min', prettyName: 'Barrier Damage Per 10 Min', weight: 1.25, required: false},
            {name: 'assault_kills_avg_per_10_min', prettyName: 'Assault Kills Per 10 Min', weight: 1.2, required: false},
            {name: 'artillery_kills_avg_per_10_min', prettyName: 'Artillery Kills Per 10 Min', weight: 1.0, required: false},
            {name: 'tactical_grenade_kills_avg_per_10_min', prettyName: 'Grenade Kills Per 10 Min', weight: 1.0, required: false},
            {name: 'recon_kills_avg_per_10_min', prettyName: 'Recon Kills Per 10 Min', weight: 1.0, required: false}
        ];
    }
    else if (hero == 'hanzo') {
        fields = [
        ...
        fields = [
            ...
            {name: 'critical_hits_avg_per_10_min', prettyName: 'Critical Hits Per 10 Min', weight: 1.25, required: false},
        ...
        fields = [
            ...
            {name: 'charged_shot_accuracy', prettyName: 'Railgun Charged Accuracy', weight: 1.25, required: false},
...
/*
    ...
    if (!fields || fields.length === 0) {
        fields = [
            { name: 'eliminations_avg_per_10_min', prettyName: 'Eliminations Per 10 Min', weight: 1.0, required: false },
            { name: 'final_blows_avg_per_10_min', prettyName: 'Final Blows Per 10 Min', weight: 1.0, required: false },
            { name: 'hero_damage_done_avg_per_10_min', prettyName: 'Hero Damage Per 10 Min', weight: 1.0, required: false },
            { name: 'healing_done_avg_per_10_min', prettyName: 'Healing Done Per 10 Min', weight: 1.0, required: false },
            { name: 'solo_kills_avg_per_10_min', prettyName: 'Solo Kills Per 10 Min', weight: 1.0, required: false },
            { name: 'critical_hits_avg_per_10_min', prettyName: 'Critical Hits Per 10 Min', weight: 1.0, required: false },
            { name: 'objective_kills_avg_per_10_min', prettyName: 'Objective Kills Average', weight: 1.0, required: false }
        ];
    }
...
function hasRequiredFieldsForHero(heroStats, hero, playMode) {
    if (!heroStats || typeof heroStats !== 'object') return false;
    return ('time_played' in heroStats) ||
           ('eliminations_avg_per_10_min' in heroStats) ||
           ('eliminations' in heroStats) ||
           ('hero_damage_done_avg_per_10_min' in heroStats) ||
           ('all_damage_done_avg_per_10_min' in heroStats) ||
           ('healing_done_avg_per_10_min' in heroStats);
}

function getAttr(heroStats, attr) {
    if (!(attr in heroStats) || heroStats[attr] === null || heroStats[attr] === undefined) {
        if (attr === 'defensive_assists_avg_per_10_min' || attr === 'offensive_assists_avg_per_10_min') {
            if ('assists_avg_per_10_min' in heroStats) return parseFloat(heroStats['assists_avg_per_10_min']);
            if ('assists' in heroStats) return parseFloat(heroStats['assists']);
        }
        if (attr === 'hero_damage_done_avg_per_10_min' || attr === 'barrier_damage_done_avg_per_10_min') {
            if ('all_damage_done_avg_per_10_min' in heroStats) return parseFloat(heroStats['all_damage_done_avg_per_10_min']);
            if ('all_damage_done' in heroStats) return parseFloat(heroStats['all_damage_done']);
        }
        ...
        if (attr == 'time_played') {
            if (typeof heroStats[attr] === 'number') {
                return Math.round(heroStats[attr] / 60);
            }
            var valStr = String(heroStats[attr]);
            if (!valStr.includes(':')) {
                var sec = parseFloat(valStr);
                return isNaN(sec) ? 0.0 : Math.round(sec / 60);
            }
            var totalMins = 0;
            var timeParts = valStr.split(':');
...
StatsEngine.prototype.stripSubStatCategories = function() {
    ...
    for (var i = 0; i < 2; i++) {
        for (var player in this.rawStats) {
            if (!this.rawStats[player] || !this.rawStats[player]["heroStats"] || !this.rawStats[player]["heroStats"][playModes[i]]) continue;
            for (var hero in this.rawStats[player]["heroStats"][playModes[i]]) {
                ...
                for (var j = 0; j < subStatCategories.length; j++) {
                    ...
                    if (typeof this.rawStats[player]["heroStats"][playModes[i]][hero][subStatCat] === 'object' && this.rawStats[player]["heroStats"][playModes[i]][hero][subStatCat] !== null) {
...
StatsEngine.prototype.getBestPlayerFit = function (comp, players, timePlayed, gameMode) {
    ...
    for (var i = 0; i < comp.length; i++) {
        for (var j = 0; j < players.length; j++) {
            ...
            var heroKey = comp[i].toLowerCase();
            if (heroKey in this.sortedStats[gameMode]) {
                playerStatsForHero = this.sortedStats[gameMode][heroKey].find(function (obj) {
            ...
            if (playerStatsForHero && (playerStatsForHero.time_played > timePlayed)) {
                bestFitResults.push({
                    heroName: comp[i],
                    name: playerStatsForHero.name,
                    overall: playerStatsForHero.overall,
                    time_played: playerStatsForHero.time_played
                });
            }
        }
...
StatsEngine.prototype.getBestPlayerFitForMaximumOverallTeamSkill = function (comp, players, timePlayed, gameMode) {
    ...
    if (!comp || comp.length === 0 || !players || players.length === 0) {
        return { error: "Please select at least 1 hero and 1 player." };
    }

    var NUM_OF_HEROES_TO_PICK = Math.min(comp.length, players.length, 5);
    ...
    var heroCombinations = Combinatorics.combination(comp, NUM_OF_HEROES_TO_PICK).toArray();
    var playerPermutations = Combinatorics.permutation(players, NUM_OF_HEROES_TO_PICK).toArray();

    var maxOverallTeamSkill = -1.0;
    var maxOverallTeam = [];
    ...
    for (var i = 0; i < heroCombinations.length; i++) {
        ...
        for (var j = 0; j < playerPermutations.length; j++) {
            ...
            for (var k = 0; k < heroComb.length; k++) {
                var heroKey = heroComb[k].toLowerCase();
                var pName = playerPerm[k];
                var playerStatsForHero = null;
                ...
                if (playerStatsForHero && (playerStatsForHero.time_played > timePlayed)) {
                    ...
                    teamSkill += parseFloat(playerStatsForHero.overall);
                } else {
                    currTeam.push({
                        heroName: heroComb[k],
                        name: pName,
            ...
            if (teamSkill > maxOverallTeamSkill || maxOverallTeam.length === 0) {
